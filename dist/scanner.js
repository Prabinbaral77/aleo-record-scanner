...
export class RecordScanner extends EventEmitter {
    ...
    async scanLoop() {
        while (!this.stopped) {
            ...
            while (!this.stopped && this.currentHeight <= latestBlock) {
                const batchEnd = Math.min(this.currentHeight + this.batchAmount - 1, latestBlock);
                let blocks;
                ...
                try {
                    ...
                    this.emitError(`Failed to fetch blocks ${this.currentHeight}–${batchEnd}`, err);
    ...
    // Block processing
    // ---------------------------------------------------------------------------
    async processBlock(block, onRecord) {
        ...
        for (const confirmedTx of block.transactions) {
            ...
            for (const transition of transitions) {
                ...
                if (!transition.program.startsWith(this.programName))
                    continue;
    ...
    async fetchBlockRange(startHeight, endHeight) {
        ...
        return fetchWithRetry(url, this.maxRetries);
    }
    /**
    ...
    async fetchLatestBlockHeight() {
        ...
        const height = await fetchWithRetry(url, this.maxRetries);
        if (typeof height !== 'number') {
            throw new TypeError(`Expected a number from /latest/height but got: ${JSON.stringify(height)}`);
        }
        return height;
    }
    // ---------------------------------------------------------------------------
